...
<table class="table order-bank" width="90%" cellpadding="2">
    ...
    <tr>
        <td width="80%" style="font-size:10pt; text-align: left"><span style="font-weight: bold">Subject: Transfer of Tk. {!! number_format($salaries->sum('salary.net_salary'),2) !!}/= to respective SB Account holders.</span> <br/>
        </td>
    </tr>

    <tr>
        <td width="100%" style="font-size:10pt; text-align: left">Dear Sir,<br/>You are requested to transfer the following listed amount to BRB Hospitals Ltd.'s employees respective SB Accounts as Salary for the month of {!! $period->month_name !!} -{!! $period->calender_year !!} from our <span style="font-weight: bold">{!! $bank->id == 2 ? 'STD A/C.# 255120879' : 'STD A/C.# 0021220004315' !!}</span>  maintained with your Branch.
        </td>
    </tr>
    ...
    @if(!empty($salaries))
        ...
        <table class="table order-bank" width="90%" cellpadding="2">
            ...
            @foreach($salaries as $row)
                    ...
                    <tr>
                        <td width="40px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $sl !!}</td>
                        <td width="150px" style="border-bottom-width:1px; font-size:8pt; text-align: left">{!! $row->personal->full_name !!}</td>
                        <td width="80px" style="border-bottom-width:1px; font-size:8pt; text-align: right">{!! number_format($row->salary->net_salary ?? 0,2) !!}</td>
                        <td width="80px" style="border-bottom-width:1px; font-size:8pt; text-align: right">{!! $bank->id == 1 ? 'AIBL' : 'DBBL' !!}</td>
            ...
            <tfoot>
                ...
                <tr>
                    ...
                    <td colspan="4" style="border-bottom-width:1px; font-size:8pt; text-align: right; font-weight: bold">{!! convert_number_to_words($salaries->sum('salary.net_salary')) !!} Taka Only</td>
                </tr>
